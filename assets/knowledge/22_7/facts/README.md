...
- `discography.json`: Sony Music official discography data, including edition
  ...
  metadata.
- `prompt-facts-brief.txt`: compact member, character, and single/album release
  facts for prompt injection.
...
needed slice of `song-title-index.txt`. Use `discography.json` for programmatic
lookup or when a prompt needs exact catalog/edition/source metadata.
