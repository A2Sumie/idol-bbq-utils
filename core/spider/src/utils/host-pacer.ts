 ...
 * TikTok live-page hydration probe). Crawl-round pacing spaces targets inside
 * one crawler, but it neither guarantees a floor for probe-side requests nor
 * coordinates across sessions hitting the same host — the measured WAF
 * threshold (2026-08-21 sa7 e2e probe §4: live-page hydration <= 1 req / 8s
 * on www.tiktok.com) is per-host, so the budget lives here.
 *
 * Small class, injectable clock/sleep for tests, no persistence (a restart
 * resetting the budget is acceptable — the first request after boot is simply
 * allowed).
 */
 ...
 * Module-level so every consumer (live-capture sessions, future probes) shares
 * the same per-host budget.
 */
