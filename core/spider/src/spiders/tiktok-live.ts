 ...
 * `LiveRoom`). Field paths are from the 2026-08-21 sa7 e2e probe and were
 * verified against its captured evidence page:
 *
 ...
 *
 * HLS is the only pull format selected here: the page's flv variants are
 * enhanced-FLV HEVC, which ffmpeg < 7.2 cannot demux, while the hls variants
 * are h264/aac and record cleanly with `ffmpeg -c copy` (sa7 §2).
 */
 ...
 * see `tiktokLivePagePacer` in utils/host-pacer.
 *
 * Transport is the curl backend: this repo's HTTPClient notes that TikTok
 * fingerprints and 429s the runtime fetch TLS/HTTP2 stack while leaving
 * curl-like clients alone.
 */
export async function probeTikTokLiveStatus(
    ...
    try {
        if (options.fetchPage) {
            ...
            const res = await HTTPClient.download_webpage_curl(url, headers, {
